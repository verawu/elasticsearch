# IVF-SQ vs int8_hnsw Benchmark Results

**Date**: April 2026
**Branch**: feat/ivf-pq-vectors
...
- **ivfpq (IVF-SQ)**: nlist=sqrt(n), nprobe=min(nlist,32), sqBits=7, early termination enabled, rerank disabled
...
### Recall sweep (int8_hnsw numCandidates / ivfpq nprobe)
...
**ivfpq** (decreasing nprobe to lower recall):
...
**No iso-recall intersection**: int8_hnsw can't reach 0.90, ivfpq can't drop below 0.90 for this data distribution.
...
**IVF-SQ wins on ALL metrics at 1M scale** — better recall, faster queries, 6x faster indexing, and smaller index size.

---

## 1M Vectors, 768d, Clustered (64 clusters, stddev=0.05)

| Metric | ivfpq (rerank) | ivfpq (no rerank) | int8_hnsw |
|---|---|---|---|
...
| 100K-500K | int8_hnsw | ivfpq (clustered) | HNSW still fast but recall degrades for clustered data |
| **~500K-1M** | **crossover** | **ivfpq** | HNSW graph quality degrades, build time explodes |
| >1M | **ivfpq** | **ivfpq** | HNSW can't maintain graph quality, memory pressure, broken recall |

## Key Findings

1. **IVF-SQ scales better than HNSW on every dimension** (recall, latency, memory, index time) once datasets cross ~500K-1M vectors
...
5. **IVF-SQ indexing is 3-6x faster** than int8_hnsw across all scales (no graph construction)
6. **At >1M/768d, int8_hnsw can't even build the index** in reasonable memory (vectors + graph > 12GB)
7. **IVF-SQ recall actually improves with scale** for clustered data (more vectors per cluster = better coverage)
