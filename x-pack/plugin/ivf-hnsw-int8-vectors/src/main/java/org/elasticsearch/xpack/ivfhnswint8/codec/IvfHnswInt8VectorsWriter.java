...
public class IvfHnswInt8VectorsWriter extends KnnVectorsWriter {

    private static final String IVF_HNSW_INT8_COMPONENT = "IvfHnswInt8Vectors";
    ...
    public IvfHnswInt8VectorsWriter(SegmentWriteState state, int nlist, int sqBits, int trainingThreshold, int kmeansIters)
        throws IOException {
        this.state = state;
        this.nlist = nlist;
    ...
    private void writeField(FieldInfo fieldInfo, float[][] vectors, int[] docIds) throws IOException {
        ...
        if (state.infoStream.isEnabled(IVF_HNSW_INT8_COMPONENT)) {
            state.infoStream.message(
                IVF_HNSW_INT8_COMPONENT,
